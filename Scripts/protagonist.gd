...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("attack"):#initiate attack
		input_enabled= 0
		area_collision.monitoring=true
		var attack_type = randi() % 2 + 1
		animated_sprite.play("knight_attack_"+str(attack_type))
	
	if x_direction>0 and input_enabled:#moving right
		animated_sprite.play("knight_running")
		animated_sprite.flip_h = false
		area_collision.scale = abs(area_collision.scale)
	elif x_direction<0 and input_enabled:#moving left
		animated_sprite.play("knight_running")
		animated_sprite.flip_h = true
		area_collision.scale = -abs(area_collision.scale)
	
	if y_direction!=0 and input_enabled:#moving up or down
		animated_sprite.play("knight_running")
	
	if x_direction==0 and y_direction==0 and input_enabled:#fully still
