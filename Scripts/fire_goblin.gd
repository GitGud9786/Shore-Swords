...
func take_damage(damage):
	...
	if HEALTH<=0:
		...
		animated_sprite.play("firegoblin_dead")
...
func _process(delta: float) -> void:
	...
	if velocity != Vector2.ZERO and attack_mode==false and dead==false:
		animated_sprite.play("firegoblin_running")
...
func lock_on_protagonist(body) -> void:
	...
	if global_position.distance_to(body.global_position)<60.0 and !body.get_death_status():
		attack_protagonist()
...
func attack_protagonist() -> void:
	attack_mode=true
	velocity = Vector2.ZERO
	animated_sprite.play("firegoblin_attack")
...
func _on_animated_sprite_2d_animation_finished() -> void:
	if animated_sprite.animation == "firegoblin_attack":
		animated_sprite.play("firegoblin_idle")
		attack_mode=false
